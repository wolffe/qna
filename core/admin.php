...
class QA_Core_Admin extends QA_Core {
	...
	function init() {
		...
		add_action( 'admin_notices', array(
			$this,
			'notice_settings'
		) );            // Notice admin to make some settings
	...
	function add_column( $columns ) {
		...
		$columns = array(
			...
			'title'       => __( 'Title' ),
			'qa-question' => __( 'Question', QA_TEXTDOMAIN ),
			'author'      => __( 'Author' ),
			'date'        => __( 'Date' ),
	...
	function notice_settings() {
		...
		if ( 'assign' != @$qa_general_settings["method"] && ! get_option( 'users_can_register' ) ) /* translators: %s means settings here */ {
	...
	function init_defaults() {
		...
		if ( ! $visitor_role ) {
			add_role( 'visitor', 'QA Visitor', array(
				'read_questions'                => true,
				'publish_questions'             => true,
				'immediately_publish_questions' => true,
				'read_answers'                  => true,
	 ...
	 */
	function admin_menu() {
		$this->hook_suffix = add_submenu_page( 'edit.php?post_type=question', __( 'Settings', QA_TEXTDOMAIN ), __( 'Settings', QA_TEXTDOMAIN ), 'manage_options', 'qa_settings', array(
			&$this,
