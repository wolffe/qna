...
abstract class Sabai_Addon_Entity_Controller_ListEntities extends Sabai_Controller
{
    ...
    protected function _doExecute(Sabai_Context $context)
    {
        ...
        // Init URL params
        $url_params = $url_params_without_filter = isset($current_sort) && $context->getRequest()->has('sort')
    ...
    protected function _getQuery(Sabai_Context $context, $sort, Sabai_Addon_Entity_Model_Bundle $bundle = null)
    {
        ...
        if ($sort) {
            if ($sort === 'random') {
                if (!$random_seed = $this->getPlatform()->getCache('entity_random_seed')) {
                    $random_seed = mt_rand(0, 9999);
                    $this->getPlatform()->setCache($random_seed, 'entity_random_seed', $this->Filter('entity_random_seed_lifetime', 3600));
                }
                $query->sortByRandom($random_seed);
