...
<?php   if (!empty($search['suggest_question'])): $num = isset($search['suggest_question_num']) ? $search['suggest_question_num'] : 5;?>
    var questions = new Bloodhound({
        ...
        remote: {
            wildcard: 'QUERY',
            url: '<?php echo $this->Url('/sabai/questions/questionlist', array('query' => 'QUERY', Sabai_Request::PARAM_CONTENT_TYPE => 'json') + (isset($bundle) ? array('bundle' => $bundle->name, 'category' => $category, 'num' => $num) : (isset($bundles) ? array('bundle' => implode(',', $bundles), 'num' => $num) : array('bundle_type' => 'questions', 'num' => $num))), '', '&');?>'
        },
        limit: <?php echo $num;?>
    });
